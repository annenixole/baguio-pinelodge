...
export default function ExpGuest() {
  ...
  // Apply filters
  useEffect(() => {
    console.log("ExpGuest - Applying filters:", { filterLocation, filterGuests, listingsCount: listings.length });
    let filtered = [...listings];

    // Filter by location (skip if "All Areas" or empty)
    if (filterLocation && filterLocation !== "All Areas") {
      console.log("ExpGuest - Filtering by location:", filterLocation);
    ...
    if (filterGuests && filterGuests !== "") {
      console.log("ExpGuest - Filtering by guests:", filterGuests);
