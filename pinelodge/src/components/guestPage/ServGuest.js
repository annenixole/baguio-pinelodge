...
export default function ServGuest() {
    ...
    const fetchServices = async () => {
      try {
        ...
        if (location.state?.searchFilters) {
          ...
          setFilteredListings(publishedServices);
  ...
  useEffect(() => {
    console.log("ServGuest - Applying filters:", { appliedFilters, listingsCount: listings.length });
    let filtered = [...listings];

    // Filter by location (skip if "All Areas" or empty)
    if (appliedFilters.location && appliedFilters.location !== "All Areas") {
      console.log("ServGuest - Filtering by location:", appliedFilters.location);
      filtered = filtered.filter((listing) => {
        ...
        console.log("ServGuest - Listing:", listing.title, "Area:", listingArea, "Matches:", matches);
        return matches;
      });
      console.log("ServGuest - After location filter:", filtered.length);
    }

    // Filter by guests (skip if empty - "Any guests")
    if (appliedFilters.guests && appliedFilters.guests !== "") {
      console.log("ServGuest - Filtering by guests:", appliedFilters.guests);
      ...
      filtered = filtered.filter((listing) => {
        ...
        console.log("ServGuest - Listing maxGuests:", listingCapacity, "Required:", guestCount, "Matches:", matches);
        return matches;
      });
      console.log("ServGuest - After guests filter:", filtered.length);
    }

    // Note: Date filtering is not applied here as we don't filter by dates
    // Dates are used for booking purposes only

    console.log("ServGuest - Final filtered count:", filtered.length);
  ...
  const handleBackToAllListings = () => {
    ...
    navigate('/ServGuest', { replace: true });
  };
