...
export default function AccomGuest() {
  ...
  useEffect(() => {
    const fetchAccommodations = async () => {
      try {
        ...
        const allAccommodations = querySnapshot.docs.map((doc) => {
          const pathSegments = doc.ref.path.split('/');
          const hostEmail = pathSegments[1];
          return { id: doc.id, hostEmail, ...doc.data() };
        });

        // Filter only published accommodations
        const publishedAccommodations = allAccommodations.filter(
          (listing) => listing && listing.status === "published"
        );
        
        setListings(publishedAccommodations);
        ...
        if (location.state?.searchFilters) {
          ...
          setFilteredListings(publishedAccommodations);
  ...
  useEffect(() => {
    ...
    if (filterLocation && filterLocation !== "All Areas") {
      ...
      console.log("AccomGuest - After location filter:", filtered.length);
    }
    ...
    if (filterGuests && filterGuests !== "") {
      ...
      filtered = filtered.filter((listing) => {
        ...
        console.log("AccomGuest - Listing:", listing.title, "Capacity:", listingCapacity, "Required:", guestCount, "Matches:", matches);
        return matches;
      });
      console.log("AccomGuest - After guests filter:", filtered.length);
    }
  ...
  const handleBackToAllListings = () => {
    ...
    navigate('/AccomGuest', { replace: true });
  };
