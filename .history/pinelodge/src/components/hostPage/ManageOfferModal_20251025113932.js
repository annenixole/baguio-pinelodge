...
export default function ManageOfferModal({
  ...
  onClose,
  activePromotion,
  listingPrice = 0, // pass the base price of the listing here
}) {
  ...
  // Populate form data depending on mode
  useEffect(() => {
    if (activePromotion) {
      setFormData({
  ...
  // Update fields + auto-update actualDiscountedPrice
  ...
  // Allow hosts to generate a code
