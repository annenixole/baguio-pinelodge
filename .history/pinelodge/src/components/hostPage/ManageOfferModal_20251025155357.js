...
export default function ManageOfferModal({
  ...
  activePromotion,
  listingId,
  listingType = "accommodations", // ✅ dynamic or defaults to accommodations
  listingPrice = 0,
  listingAvailability,
}) {
  ...
  // ✅ dynamically calculate discounted price
  ...
  // ✅ populate existing promo if editing
  useEffect(() => {
    if (activePromotion) {
      setFormData({
  ...
  // ✅ allow host to generate random promo code
  ...
  const handleSubmit = async () => {
    try {
      ...
      await updateDoc(docRef, {
        promotion: promotionData, // ✅ store promo directly under the listing doc
      });
