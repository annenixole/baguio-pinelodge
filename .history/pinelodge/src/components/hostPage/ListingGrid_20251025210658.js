...
export default function ListingGrid({ typeFilter, setSelectedIndex, reloadTrigger }) {
  ...
  const [statusFilter, setStatusFilter] = useState("published"); // default
  ...
  // Set up a listener for each type of listing
  types.forEach((type) => {
    const collectionRef = collection(db, "users", user.email, type);
    const unsubscribe = onSnapshot(collectionRef, (snapshot) => {
      setListings((prevListings) => {
        // Remove old listings of this type
        const filtered = prevListings.filter((item) => item.type !== type.slice(0, -1));
