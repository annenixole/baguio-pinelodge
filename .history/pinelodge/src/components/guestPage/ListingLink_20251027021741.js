            ...
            if (listingParam) {
                try {
                    ...
                    if (snap.exists()) {
                        ...
                        const hostDocRef = doc(db, "users", listingInfo.hostEmail);
                        const hostSnap = await getDoc(hostDocRef);
                        
                        if (hostSnap.exists()) {
                            const hostData = hostSnap.data();
                            const fullName = `${hostData.firstName || ""} ${hostData.lastName || ""}`.trim();
                            setHostName(fullName || "Host");
                            console.log("✓ Fetched host name:", fullName);
                        } else {
                            setHostName("Host");
