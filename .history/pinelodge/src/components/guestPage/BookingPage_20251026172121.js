...
export default function BookingPage() {
    ...
    const [bookingRange, setBookingRange] = useState(type === "accommodation" ? [new Date(), new Date()] : [new Date()]);
                ...
                if (listing?.type && listing?.id && listing?.hostEmail) {
                    ...
                    if (snap.exists()) {
                        ...
                        if (data.availability?.start && data.availability?.end) {
                            ...
                            console.log("📅 Availability Range:", startLocal, endLocal);
                            setAvailabilityStart(startLocal);
                            setAvailabilityEnd(endLocal);
                        } else {
                            console.warn("No availability field found in Firestore listing");
                        }
                    }
                }
            } catch (error) {
                            ...
                            {(availabilityStart || availabilityEnd) && (
                                <Box sx={{ mb: 2, p: 1, bgcolor: "#f0f0f0", borderRadius: 1 }}>
                                    <Typography variant="caption">
                                        Available from: {availabilityStart ? availabilityStart.toLocaleDateString() : "Not set"}
                                        {" to "}
                                        {availabilityEnd ? availabilityEnd.toLocaleDateString() : "Not set"}
                                    </Typography>
                                </Box>
                            )}
                            {/* ✅ MUI Date Picker (FireStore-limited like expiry date) */}
                            <LocalizationProvider dateAdapter={AdapterDateFns}>
                                {type === "accommodation" ? (
                                    <Box sx={{ display: "flex", gap: 2, justifyContent: "center", maxWidth: 450, mx: "auto" }}>
                                        {/* Check-in */}
                                        <DatePicker
                                            label="Check-in"
                                            value={bookingRange[0] || null}
                                            onChange={(newDate) =>
                                                setBookingRange([newDate, bookingRange[1] || null])
                                            }
                                        ...
                                        <DatePicker
                                            label="Check-out"
                                            value={bookingRange[1] || null}
                                            onChange={(newDate) =>
                                                setBookingRange([bookingRange[0] || null, newDate])
                                            }
                                            minDate={availabilityStart}
                                            maxDate={availabilityEnd}
                                            slotProps={{
                                    ...
                                    // ✅ For experiences/services — single date
                                    <Box sx={{ display: "flex", justifyContent: "center", maxWidth: 300, mx: "auto" }}>
                                        <DatePicker
                                            label="Select Date"
                                            value={bookingRange[0] || null}
                                            onChange={(newDate) => setBookingRange([newDate])}
                                            minDate={availabilityStart}
                                            maxDate={availabilityEnd}
                                            slotProps={{
                                                textField: { fullWidth: true, variant: "outlined" },
                                ...
                                {bookingRange && bookingRange.length > 0 ? (
                                    type === "accommodation" && bookingRange.length === 2 ? (
                                        <Typography sx={{ color: "#70873F", fontWeight: 600 }}>
                                            {`${bookingRange[0].toLocaleDateString()} → ${bookingRange[1].toLocaleDateString()}`}
                                        </Typography>
                                    ) : (
                                        <Typography sx={{ color: "#70873F", fontWeight: 600 }}>
                                            {bookingRange[0].toLocaleDateString()}
                                        </Typography>
                                    )
                                ) : (
