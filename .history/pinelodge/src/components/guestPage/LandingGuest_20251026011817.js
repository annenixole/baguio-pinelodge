 ...
 useEffect(() => {
  const fetchListings = async () => {
    try {
      ...
      // ✅ Explicitly extract promotions (if any)
      const parseDocs = (snap) =>
        snap.docs.map((doc) => {
          ...
          return {
            id: doc.id,
            ...data,
            promotion: data.promotion || null, // include promotion explicitly
          };
        });

      const allListings = [
        ...parseDocs(querySnapshot),
        ...parseDocs(serviceSnapshot),
        ...parseDocs(expSnapshot),
      ];

      // ✅ Filter only published listings
      setListings(allListings.filter((l) => l && l.status === "published"));
    } catch (error) {
