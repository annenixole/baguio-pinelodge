                ...
                if (listing?.type && listing?.id && listing?.hostEmail) {
                    ...
                    if (snap.exists()) {
                        ...
                        if (data.availability?.start && data.availability?.end) {
                            setAvailabilityStart(new Date(data.availability.start.toDate()));
                            setAvailabilityEnd(new Date(data.availability.end.toDate()));
                        }
                    }
                }
            } catch (error) {
                        ...
                        <Box sx={{ p: 4, backgroundColor: "#fff", }}>
                            ...
                            <Box
                                sx={{
                                    ...
                                    // ✅ Style for disabled calendar tiles
                            ...
                            >
                                <Calendar
                                    ...
                                    value={bookingRange}
                                    // ✅ Hard limit the visible and clickable dates
                                    minDate={availabilityStart || new Date()}
                                    maxDate={availabilityEnd || null}
                                    // ✅ Disable all dates outside the Firestore range
                                    tileDisabled={({ date, view }) => {
                                        if (view === "month") {
                                            if (availabilityStart && date < availabilityStart) return true;
                                            if (availabilityEnd && date > availabilityEnd) return true;
                                        }
                                        return false;
                                    }}
                                    // ✅ Prevent navigation to months outside availability
                                    onActiveStartDateChange={({ activeStartDate, action }) => {
                                        if (action === "prev" && availabilityStart && activeStartDate < availabilityStart) {
                                            setTimeout(() => {
                                                document.querySelector(".react-calendar__navigation__next-button")?.click();
                                            }, 0);
                                        }
                                        if (action === "next" && availabilityEnd && activeStartDate > availabilityEnd) {
                                            setTimeout(() => {
                                                document.querySelector(".react-calendar__navigation__prev-button")?.click();
                                            }, 0);
                                        }
                                    }}
                                />
