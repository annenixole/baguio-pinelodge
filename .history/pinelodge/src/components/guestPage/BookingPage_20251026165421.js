                ...
                <Box sx={{ flex: 2, maxWidth: 700 }}>
                    {showDatePicker ? (
                        ...
                        <Box sx={{ p: 4, backgroundColor: "#fff", }}>
                            ...
                            {/* ✅ Calendar with Firestore-limited range */}
                            <Box
                                sx={{
                                    display: "flex",
                            ...
                            >
                                <Calendar
                                    ...
                                    onChange={(value) => {
                                        if (type === "accommodation") setBookingRange(value);
                                        else setBookingRange([value]);
                                    }}
                                    value={bookingRange}
                                    minDate={availabilityStart || new Date()}
                                    maxDate={availabilityEnd || null}
                                    // ✅ Disable tiles outside range
                                    tileDisabled={({ date }) =>
                                        (availabilityStart && date < availabilityStart) ||
                                        (availabilityEnd && date > availabilityEnd)
                                    }
                                    // ✅ Prevent navigation to months outside range
                                    onActiveStartDateChange={({ activeStartDate }) => {
                                        if (availabilityStart && activeStartDate < availabilityStart) {
                                            // force back to min month
                                            setTimeout(() => {
                                                document
                                        ...
                                        if (availabilityEnd && activeStartDate > availabilityEnd) {
                                            // force back to max month
                                            setTimeout(() => {
                                                document
