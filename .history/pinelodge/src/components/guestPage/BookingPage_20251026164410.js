            ...
            try {
                if (listing?.type && listing?.id && listing?.hostEmail) {
                    ...
                    if (snap.exists()) {
                        ...
                        console.log("Availability object:", data.availability);
                        
                        if (data.availability?.start && data.availability?.end) {
                            // Handle both Firestore Timestamp and regular Date strings
                            let startDate, endDate;
                            
                            if (typeof data.availability.start.toDate === 'function') {
                                startDate = data.availability.start.toDate();
                            } else {
                                startDate = new Date(data.availability.start);
                            }
                            
                            if (typeof data.availability.end.toDate === 'function') {
                                endDate = data.availability.end.toDate();
                            } else {
                                endDate = new Date(data.availability.end);
                            }
                            
                            console.log("Setting availability - Start:", startDate, "End:", endDate);
