...
export default function BookingPage() {
    ...
    useEffect(() => {
        const fetchAvailability = async () => {
            try {
                ...
                console.log("📧 Host email:", listing?.hostEmail);
                console.log("🏷️ Listing type:", listing?.type);
                console.log("🆔 Listing id:", listing?.id);

                if (listing?.type && listing?.id && listing?.hostEmail) {
                    const ref = doc(db, "users", listing.hostEmail, `${listing.type}s`, listing.id);
                    console.log("📍 Firestore path:", `users/${listing.hostEmail}/${listing.type}s/${listing.id}`);
                    ...
                    if (snap.exists()) {
                        ...
                        console.log("📅 Availability field:", data.availability);

                        if (data.availability?.start && data.availability?.end) {
                            // ✅ Convert Firestore Timestamps safely to local JS Date (ignore timezone shift)
                            const start = data.availability.start.toDate();
                            const end = data.availability.end.toDate();

                            // ✅ Force them to local calendar boundaries
                            ...
                            // ✅ If start date is in the past or today, use today as the minimum date
                            const effectiveStartDate = startLocal <= today ? today : startLocal;
                            const wasAdjusted = startLocal < today;

                            console.log("📅 Original start date:", startLocal);
                            console.log("📅 Today's date:", today);
                            console.log("🔍 Is start date in past?", startLocal < today);
                            console.log("✅ Effective start date (limited to today if past):", effectiveStartDate);
                            console.log("✅ Availability Range SET:", effectiveStartDate, endLocal);
                            
                            setAvailabilityStart(effectiveStartDate);
                            setAvailabilityEnd(endLocal);
                            setIsDateAdjusted(wasAdjusted);
                        } else {
                            console.warn("⚠️ No availability field found in Firestore listing");
                            console.warn("⚠️ Available fields:", Object.keys(data));
                        }
                    } else {
                        console.warn("⚠️ Document does not exist at path");
                    }
                } else {
                    console.warn("⚠️ Missing required listing fields:", {
                        hasType: !!listing?.type,
                        hasId: !!listing?.id,
                        hasHostEmail: !!listing?.hostEmail
                    });
                }
            } catch (error) {
    ...
    return (
        <>
                        ...
                        <Box sx={{ p: 4, backgroundColor: "#fff", }}>
                            ...
                            <Box sx={{ mb: 2, p: 2, bgcolor: "#f0f9ff", borderRadius: 1, border: "1px solid #0ea5e9" }}>
                                ...
                                <Typography variant="body2" sx={{ color: "#0c4a6e" }}>
                                    Available from: {availabilityStart ? availabilityStart.toLocaleDateString(undefined, {
                                        year: "numeric",
                                        month: "long",
                                        day: "numeric",
                                    }) : "Not set"}
                                ...
                                <Typography variant="body2" sx={{ color: "#0c4a6e" }}>
                                    Available until: {availabilityEnd ? availabilityEnd.toLocaleDateString(undefined, {
                                        year: "numeric",
                                        month: "long",
                                        day: "numeric",
                                    }) : "Not set"}
                                </Typography>
                                {availabilityStart && availabilityStart.toDateString() === new Date().toDateString() && (
                                    <Typography variant="caption" sx={{ color: "#0369a1", mt: 1, display: "block", fontStyle: "italic" }}>
                                        ⏰ Start date adjusted to today (original date was in the past)
                                    </Typography>
                                )}
                            </Box>
                    ...
                    <Card sx={{ p: 3 }}>
                        <CardContent>
                            ...
                            <Box sx={{ mt: 3 }}>
                                ...
                                <Typography sx={{ color: "text.secondary", ml: 1 }}>2:30 pm</Typography>
                                {/* ✅ Discount Section */}
                                <Divider sx={{ my: 2 }} />
            ...
            </Card> {/* ✅ closes the unified card layout */}
        </>
    );
}
