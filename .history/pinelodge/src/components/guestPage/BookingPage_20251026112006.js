...
export default function BookingPage() {
    ...
    useEffect(() => {
        const fetchHostName = async () => {
            try {
                if (listing?.hostId) {  // assumes hostId is stored in the listing document
                    const hostRef = doc(db, "users", listing.hostId); // adjust collection name if needed
                    const hostSnap = await getDoc(hostRef);

                    if (hostSnap.exists()) {
                        setHostName(hostSnap.data().name || "Host");
                    } else {
