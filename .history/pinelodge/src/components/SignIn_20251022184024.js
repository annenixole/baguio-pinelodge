import burnham from '../elements/burnhamPark.jpg';
...
export default function SignIn() {
  ...
  const location = useLocation(); // ✅ Properly define from React Router
  ...
  // ✅ Detect email verification redirect (e.g. /SignIn?verified=true)
  useEffect(() => {
    ...
    if (params.get("verified") === "true") {
      ...
      // Remove query param from the URL (no reload)
  ...
  const handleSignin = async (e) => {
    ...
    try {
        ...
        setTimeout(() => {
          if (userData.role === "host") {
            if (userData.isNewUser === true)
              navigate("/hostPage/GetStarted");
            else
              navigate("/hostPage/HomeHost");
          } else {
            navigate("/");
          }
        }, 1500);
      ...
      else if (error.code === "auth/invalid-email" || error.code === "auth/missing-password")
        setMsg("Input all fields");
