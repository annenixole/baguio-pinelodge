...
export default function SignUp() {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      const verificationToken = btoa(`${userCredential.user.uid}-${Date.now()}`);
      const verificationLink = `${window.location.origin}/verifyEmail?token=${verificationToken}`;
      
      // Save to Firestore with verification token
