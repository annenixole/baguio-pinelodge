...
export default function LandingPage() {
  ...
  return (
    <>
      ...
      {/* ABOUT PINELODGE */}
      <Box sx={{ py: 8, textAlign: "center", backgroundColor: "#ffffffff", mt: 4}}>
        <Typography variant="h4" sx={{ fontWeight: 700, mb: 4, color: "#30410D" }}> Your Ideal Baguio Getaway Awaits </Typography>
        <Grid container spacing={3} justifyContent="center">
          {[
            { icon: <FavoriteIcon sx={{ fontSize: 40, color: "#a54646ff;" }} />, title: "Comfort & Warmth", text: (<>Retreat to a cozy escape <br/> surrounded by pine-scented air</>) },
            { icon: <StarIcon sx={{ fontSize: 40, color: "#DE7001;" }} />, title: "Top-Rated Hosts", text: (<>Highly rated hosts <br/> ensuring quality and care</>)},
            { icon: <LocalOfferIcon sx={{ fontSize: 40, color: "#0f6420ff;" }} />, title: "Best Deals", text: (<>Affordable stays <br/> with exclusive seasonal offers</>)},
            { icon: <TravelExploreIcon sx={{ fontSize: 40, color: "#4b66b3ff;" }} />, title: "Perfect Location", text:(<>Find spots near parks, <br/> cafes, and tourist landmarks</>)},
          ].map((item, index) => (
            <Grid item xs={12} sm={6} md={3} key={index}>
              <Card sx={{ borderRadius: 3, boxShadow: 2, py: 4, p:4 }}>
                <CardContent>
                  {item.icon}
